...
A registered submodule defaults to its superproject's `unfertig.json` and refuses to silently use app-owned data when that config is absent. A standalone app defaults to `APP/board/data.json`, initializing one onboarding task only if the board is missing. Existing boards are never seeded again. Explicitly configured missing boards require `--init`. The app's own migrated backlog uses `APP/unfertig-development.json` and `APP/development/`; it is separate from starter content.
