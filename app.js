...
function executionProfile(todo = {}, role = 'implementation') {
  ...
  if (requested === 'auto') {
    const order = Object.keys(effortDefinitions.profiles);
    const prior = [...(todo.workflow?.agent_runs || [])].reverse().find(entry => entry.role === role);
    if (prior?.outcome === 'repair_needed' && order.includes(prior.profile)) {
      const rank = Math.min(order.length-1, order.indexOf(prior.profile)+1);
      if (rank > order.indexOf(profile)) {profile = order[rank]; reason = 'Escalated after a failed verification or incomplete implementation; next authorized attempt only';}
    }
  }
...
function categoryEditor(todo = {}) {
  const key = todo.category || '';
  return `<label>Work category<select name="category"><option value="">Unclassified</option>${Object.entries(categoryDefinitions.categories).map(([value,d]) => `<option value="${value}" ${value === key ? 'selected' : ''}>${d.label}</option>`).join('')}${key && !categoryDefinitions.categories[key] ? `<option selected value="${escapeHTML(key)}">${escapeHTML(key)}</option>` : ''}</select></label><p class="hint category-help" style="white-space:pre-line">${escapeHTML(categoryBrief(todo))}</p>`;
}
document.addEventListener('change', event => {
  if (event.target.name === 'category') event.target.closest('form').querySelector('.category-help').textContent = categoryBrief({category:event.target.value});
});
function todoSummary(todo, {key = todo.id, project = "", priorityHTML = ""} = {}) {
  return `<summary><span class="status-icon" ${!project ? `data-workflow-icon="${escapeHTML(key)}"` : ""} role="img" aria-label="${todo.status}">${todo.status === 'closed' ? '✓' : todo.status === 'started' ? '•' : ''}</span><span class="todo-main"><span class="todo-title">${escapeHTML(todo.name)}</span><span class="todo-subtitle"><span class="mono">${todo.id}</span>${project ? `<span class="project-column">${escapeHTML(project)}</span>` : publicationBadge("todos", todo.id)}<span>· ${escapeHTML(todo.group || 'Ungrouped')}</span><span>· ${escapeHTML(categoryDefinitions.categories[todo.category]?.label || todo.category || 'Unclassified')}</span><span>· ${todo.status}</span>${todo.tags.map(tag => `<span class="tag">${escapeHTML(tag)}</span>`).join('')}</span></span>${priorityHTML || priorityControl(todo,key,compatibility.read_only)}<span class="summary-actions"><button type="button" class="button small" data-brief="${escapeHTML(key)}" aria-label="AI briefing for ${todo.id}">✧ AI briefing</button><button type="button" class="button small" data-human-brief="${escapeHTML(key)}" aria-label="Human briefing for ${todo.id}">Human briefing</button>${!project ? `<span class="workflow-next" data-workflow-next="${escapeHTML(key)}" hidden></span>` : ''}</span><span class="chevron" aria-hidden="true">›</span></summary>`;
}
function todoCard(todo) {
  ...
  <form class="todo-editor" data-id="${todo.id}"><label>Depends on (ticket IDs, comma separated)<input name="depends_on" value="${escapeHTML((todo.depends_on || []).join(', '))}"></label><p class="provenance">Entered by ${escapeHTML(todo.author)} · ${escapeHTML(date(todo.date_entered))} · Structured by ${escapeHTML(todo.created_by)}${todo.source_ideas.length ? ' · From ' + todo.source_ideas.join(', ') : ''}${todo.status === 'closed' ? ' · Closed by ' + escapeHTML(todo.closed_by) + ' on ' + escapeHTML(date(todo.date_closed)) : ''}</p>
...
function renderTodos() {
  queueMicrotask(() => document.dispatchEvent(new Event('unfertig:todos-rendered')));
  if (!data) return;
  const query = $('#search').value.trim().toLocaleLowerCase();
  const filter = $('#status-filter').value, group = $('#group-filter').value, tag = $('#tag-filter').value;
  const todos = data.todos.filter(todo => (filter === 'all' || (filter === 'active' ? todo.status !== 'closed' : todo.status === filter)) && (!group || todo.group === group) && (!tag || todo.tags.includes(tag)) && (!query || [todo.id,todo.name,todo.description,todo.author,todo.group,todo.category || '',...todo.tags,...todo.source_ideas].join(' ').toLocaleLowerCase().includes(query)));
...
function renderPreservingDrafts() {
  ...
  for (const draft of drafts) {
    ...
    if (form) { for (const [key,value] of Object.entries(draft.values)) form.elements[key].value = value; form.querySelector('.category-help').textContent = categoryBrief({category:form.elements.category.value}); form.dataset.dirty = 'true'; }
  }
...
async function briefingSnapshot(body) {
  const response = await fetch('/api/briefing', {method:'PUT',
    headers:{'Content-Type':'application/json', 'X-Board-Token':token}, body:JSON.stringify(body)});
  const result = await response.json();
  if (!response.ok) throw new Error(result.error || 'Current context could not be read.');
  return result;
}
$('#process').addEventListener('click', async () => {
  try {
    const snapshot = await briefingSnapshot({role:boardContext?.mode === 'aggregation' ? 'aggregation' : 'processing'});
    ...
    await showCopy(snapshot.context_packet + '\n\n' + processBrief(), 'All pending ideas · processing briefing');
...
document.addEventListener('click', async event => {
  ...
  if (button.dataset.brief || button.dataset.humanBrief) {
    ...
    try {
      const snapshot = await briefingSnapshot({todo_id:id});
      if (snapshot.compatibility?.read_only || snapshot.history?.pending) throw new Error('Resolve compatibility or pending history before copying.');
      ...
      if (button.dataset.humanBrief) {
        showCopy(humanBrief(todo, snapshot.data, snapshot.context, snapshot.context_packet), `Human briefing · ${id}`, 'A handoff for a person: the task, original context, and how to finish. Copying leaves the todo unchanged.');
      } else {
        showCopy(implementationBrief(todo, snapshot.data, snapshot.context, snapshot.context_packet), `AI briefing · ${id}`);
      }
    } catch (error) { toast('No current briefing copied. ' + error.message); }
  }
