...
function hasDraft() { return Boolean($('.todo-editor[data-dirty="true"]') || $('#create-dialog').open || $('#idea-text').value.trim()); }
...
$('#todos').addEventListener('submit', async event => {
  event.preventDefault(); const form = event.target, values = Object.fromEntries(new FormData(form)), author = actor(); if (!author) return;
...
function renderPreservingDrafts() {
  const drafts = $$('.todo-editor[data-dirty="true"]').map(form => ({id:form.dataset.id,values:Object.fromEntries(new FormData(form))}));
  render();
