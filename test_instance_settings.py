...
class InstanceSettingsTests(unittest.TestCase):
    ...
    def test_http_token_validation_allowlist_and_conflict(self):
        ...
        try:
            ...
            self.assertIn('class="app-version"', html)
            self.assertNotIn('__APPLICATION_BUILD__', html)
            for path in ('/instance-settings.js', '/maintenance.js'):
                with urlopen(origin+path) as response:
                    self.assertEqual(response.status, 200)
            with urlopen(origin+'/api/maintenance') as response:
            ...
            # Settings participate in the same final drain gate as board writes.
            before = self.store.config.read_bytes()
            server.maintenance.quiescing = True
            with self.assertRaises(HTTPError) as caught:
                send(server.token, {'max_workers':5}, revision=self.editor.view()['revision'])
            self.assertEqual(caught.exception.code,503); caught.exception.close()
            self.assertEqual(self.store.config.read_bytes(), before)
            self.assertEqual(server.maintenance.writers, 0)
        finally:
