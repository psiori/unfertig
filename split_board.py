...
def split(source, app, project, recovery, assignments):
    ...
    try:
        if manifest_path.exists():
            ...
            manifest = dict(source=str(source), destinations={'app':str(app), 'project':str(project)}, assignments=assignments,
        ...
        for label, target in (('app',app),('project',project)):
            ...
            for receipt in (recovery/'original'/'.receipts').glob('*.json'):
                atomic(target/'.receipts'/receipt.name, encode({'retired':True}))
        ...
        atomic(source, encode({'schema_version':0,'retired':True,'manifest':str(manifest_path)}))
