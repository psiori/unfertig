...
test('bulk review uses complete owner set, confirms exact commits and reports partial rejection',async()=>{
  ...
  assert.match(h.button.textContent,/Merge all PRs & push/);
  assert.match(h.confirms[0],/instance maintenance follows publication independently/);
  assert.doesNotMatch(h.confirms[0],/Merge & restart|restarts configured artifacts/);
