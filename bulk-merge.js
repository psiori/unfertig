...
(() => {
  ...
  function render() {
    ...
    button.textContent = running ? 'Reviewing / queueing…' : pending ? 'Retry reviewed merge batch' : 'Merge all PRs & push';
  }
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      if (!pending) {
        ...
        const notice = `Queue ${entries.length} finished todos for Merge & push?\n\n${commits}\n\nEach action integrates and publishes its changed repositories through the serialized queue. Configured instance maintenance follows publication independently. Combined tests, migration checks and recovery still apply; entries can fail separately.${review.queue_blocked_by ? '\nQueue paused by '+review.queue_blocked_by+'. Existing recovery controls remain required.' : ''}${omissions ? '\n\nExcluded:\n'+omissions : ''}`;
        message(notice);
