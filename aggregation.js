...
renderTodos = function() {
  ...
  const card = ({source:s,todo:t}) => {
    ...
    return `<details class="todo ${t.status}" data-record-key="${escapeHTML(key)}" data-id="${escapeHTML(key)}" ${expanded.has(key) ? 'open' : ''}>${todoSummary(t,{key,project:sourceName(s),priorityHTML:priorityControl(t,key,s.status!=='reachable'||s.compatibility?.read_only||s.history?.pending)})}<section class="source-details"><p class="provenance">Requested by ${escapeHTML(t.author)} · structured by ${escapeHTML(t.created_by)} · ${escapeHTML(t.id)}</p><p class="source-description">${escapeHTML(t.description)}</p><p class="hint" style="white-space:pre-line">${escapeHTML(categoryBrief(t))}</p><dl><dt>Status</dt><dd>${escapeHTML(t.status)}</dd><dt>Group</dt><dd>${escapeHTML(t.group||'Ungrouped')}</dd><dt>Tags</dt><dd>${escapeHTML(t.tags.join(', '))}</dd><dt>Implementation</dt><dd>${escapeHTML(t.commit_hash||'No commit recorded')}</dd></dl>${t.pr_url ? `<p><a href="${escapeHTML(t.pr_url)}" target="_blank" rel="noopener">Open PR ↗</a></p>` : ''}${t.commit_url ? `<p><a href="${escapeHTML(t.commit_url)}" target="_blank" rel="noopener">Open commit ↗</a></p>` : ''}<p class="hint">Priority can be edited here. Other fields are read-only; use the owning board to edit them. Briefings verify the current source before copying.</p>${s.status!=='reachable' ? '<p class="route-warning">Stale cached details · refresh the source before relying on them.</p>' : ''}${ownerLink(s,'todo',t.id)}</section></details>`;
  };
