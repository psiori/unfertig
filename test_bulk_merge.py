...
class BulkMergeTests(unittest.TestCase):
    ...
    def test_batch_uses_existing_pause_restart_recovery_and_candidate_checks(self):
        ...
        enqueue(self.workflow, batch)
        self.await_workers(); self.await_receipt(ids[0]); self.restart_coordinator(); self.workflow.tick()
        ...
        self.assertEqual(state['runs'][ids[0]]['integration_tested_commit'], state['runs'][ids[0]]['published_commit'])
        with patch.object(self.workflow, 'dispatch'):
        ...
        self.assertNotEqual(self.workflow.status()['runs'][ids[1]]['phase'], 'merge_queued')
        self.await_receipt(ids[1])
