...
class DeploymentPreflightTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config.write_text(json.dumps(dict(format_version='1.9.0', mode='embedded',
        ...
        (self.data / '.receipts/request-1234567890.json').write_text(json.dumps(dict(
            format_version='1.9.0', request_id='request-1234567890', fingerprint='original', assigned=[],
            extension={'keep': True})))
    ...
    def downgrade(self):
        ...
        path.write_text(path.read_text().replace("FORMAT_VERSION = '1.9.0'", "FORMAT_VERSION = '1.8.0'"))
    ...
    def test_18_to_19_preserves_live_board_receipts_and_config(self):
        self.downgrade()
        before = files(self.data), files(self.config.parent)
        result = assess(self.candidate, self.context)
        self.assertEqual(result['state'], 'migration_required')
        self.assertEqual(result['current_formats'], ['1.8.0'])
        self.assertEqual(result['target_format'], '1.9.0')
        self.assertEqual((files(self.data), files(self.config.parent)), before)
    ...
    def test_config_only_migration_reports_older_config_format(self):
        ...
        result = assess(self.candidate, self.context)
        self.assertEqual(result['state'], 'migration_required')
        self.assertEqual(result['current_formats'], ['1.8.0', '1.9.0'])
