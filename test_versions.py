...
class VersionTests(unittest.TestCase):
    ...
    def files(self):
        return {p.relative_to(self.root).as_posix():p.read_bytes() for p in self.root.rglob('*') if p.is_file() and p.name != '.server.lock'}
    ...
    def test_mixed_supported_versions_and_future_build(self):
        ...
        header = json.loads(self.path.read_text()); header['format_version']='1.1.9'; self.write(self.path,header)
        self.store.initialize()
        self.assertEqual(json.loads(self.todo.read_text())['format_version'],FORMAT_VERSION)
        self.assertEqual(json.loads(self.path.read_text())['format_version'],'1.1.9')
    ...
    def test_future_minor_read_only_preserves_mixed_old_bytes(self):
        data = json.loads(self.todo.read_text()); data['format_version']='1.2.0'; self.write(self.todo,data)
    ...
    def test_future_build_edit_preserves_unknown_fields_and_version(self):
        data = json.loads(self.todo.read_text()); data.update(format_version='1.1.42', extension={'nested':[1,2]})
        ...
        self.assertEqual(saved['format_version'],'1.1.42')
