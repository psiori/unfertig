...
class VersionTests(unittest.TestCase):
    ...
    def test_published_scope_step_precedes_integration_policy_defaults(self):
        from versions import MIGRATIONS, saved_scope_format, relaxed_integration_format
        self.assertIs(MIGRATIONS['1.21.0'], saved_scope_format)
        self.assertIs(MIGRATIONS['1.22.0'], relaxed_integration_format)
        published = saved_scope_format({'format_version':'1.21.0'}, 'config')
        self.assertEqual(published['format_version'], '1.22.0')
        self.assertNotIn('integration', published['workflow'])
        upgraded = migrate(published, 'config')
        self.assertEqual(upgraded['workflow']['integration'], {
            'mode':'strict', 'automatic_repair':False,
            'max_attempts':3, 'reuse_board_metadata':False})
        self.assertEqual(upgraded['workflow']['max_workers'], published['workflow']['max_workers'])
        self.assertEqual(inspect(upgraded, supported='1.22.0')[0], 'read_only')
        for kind in ('todo', 'receipt'):
            original = dict(format_version='1.22.0', execution_profile='auto',
                            scope_authorizations=[{'extension':'preserved'}],
                            workflow={'scope_attempts':[{'old':'evidence'}]},
                            request_id='retained', fingerprint='unchanged', extension=[1])
            with self.subTest(kind=kind):
                upgraded = migrate(original, kind)
                self.assertEqual(semantic(upgraded), semantic(original))
                self.assertEqual(migrate(upgraded, kind), upgraded)
                self.assertEqual(original['format_version'], '1.22.0')

    def test_relaxed_integration_config_recovery_preserves_explicit_values_and_receipts(self):
        self.store.initialize()
        old = json.loads(self.config.read_text())
        old.update(format_version='1.22.0', workflow={'integration': {'mode':'relaxed', 'automatic_repair':True, 'extension':[1]}})
        self.write(self.config, old)
        receipt = dict(format_version='1.22.0', request_id='b'*16, fingerprint='unchanged', assigned=[])
        receipt_path = self.store.receipts / ('b'*16+'.json')
        self.write(receipt_path, receipt)
        before_ideas = self.store.read()[0]['ideas']
        atomic = storage.atomic
        def interrupt(path, raw):
            if path.resolve() == self.config.resolve():
                raise OSError('Interrupted integration policy migration')
            return atomic(path, raw)
        with patch('storage.atomic', side_effect=interrupt), self.assertRaises(OSError):
            self.store.initialize()
        self.assertEqual(json.loads(self.config.read_text()), old)
        self.assertTrue(self.store.journal.exists())
        self.store.initialize()
        self.assertEqual(json.loads(self.config.read_text())['workflow']['integration'],
                         {'mode':'relaxed', 'automatic_repair':True, 'max_attempts':3, 'reuse_board_metadata':False, 'extension':[1]})
        self.assertEqual(semantic(json.loads(receipt_path.read_text())), semantic(receipt))
        self.assertEqual(self.store.read()[0]['ideas'], before_ideas)
        before=self.files();self.store.initialize();self.assertEqual(self.files(),before)
    ...
    def test_discovery_step_defaults_preserves_explicit_settings_and_old_writer_guard(self):
        ...
        for patterns in (None, [], ['../um-?/config.json']):
            ...
            self.assertEqual(migrate(result, 'config'), dict(result, format_version=FORMAT_VERSION, workflow={'max_workers':4,'after_publish':[],'integration':{'mode':'strict','automatic_repair':False,'max_attempts':3,'reuse_board_metadata':False}}, processing={'context_sources':{}}))
    ...
    def test_mixed_supported_versions_and_future_build(self):
        ...
        header = json.loads(self.path.read_text()); header['format_version']='1.23.9'; self.write(self.path,header)
        self.store.initialize()
        self.assertEqual(json.loads(self.todo.read_text())['format_version'],FORMAT_VERSION)
        self.assertEqual(json.loads(self.path.read_text())['format_version'],'1.23.9')
    ...
    def test_future_minor_read_only_preserves_mixed_old_bytes(self):
        data = json.loads(self.todo.read_text()); data['format_version']='1.24.0'; self.write(self.todo,data)
    ...
    def test_future_build_edit_preserves_unknown_fields_and_version(self):
        data = json.loads(self.todo.read_text()); data.update(format_version='1.23.42', extension={'nested':[1,2]})
        ...
        self.assertEqual(saved['format_version'],'1.23.42')
    ...
    def test_current_format_creation_defaults_and_future_effort_is_read_only(self):
        ...
        future = dict(result['data']['todos'][0], format_version='1.24.0', effort='future')
...
class WorkerDefaultMigrationTests(unittest.TestCase):
    def test_every_supported_upgrade_preserves_explicit_limits_and_extensions(self):
        from versions import MIGRATIONS
        for version in MIGRATIONS:
            for limit in (None, 1, 2, 8):
                config = dict(format_version=version, workflow={'extension':42}, extension=['keep'])
                if limit is not None: config['workflow']['max_workers'] = limit
                result = migrate(config, 'config')
                self.assertEqual(result['workflow']['max_workers'], limit if limit is not None else 4)
                self.assertEqual(result['workflow']['extension'],42)
                self.assertEqual(result['extension'],['keep'])
