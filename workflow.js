...
(() => {
  ...
  document.addEventListener('click', async event => {
    ...
    try {
      ...
      if (!requests.has(id)) requests.set(id, {id,action,...external,revision:snapshot.revisions.todos[id],commit:action === 'verify_existing' ? run?.retained_commit : run?.commit,...(action === 'migrate' ? {review_id:run.deployment_review.review_id} : {}),request_id:globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`});
