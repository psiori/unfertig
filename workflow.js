...
(() => {
  ...
  function nextStep(todo, run) {
    ...
    if (['ready','test_failed'].includes(run.phase)) return ['test',run.repositories ? 'Verify repositories' : 'Preview',true];
    if (['tested','merge_failed','push_failed','restart_failed'].includes(run.phase)) return ['merge','Merge & push',true];
