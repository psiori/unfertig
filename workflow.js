...
(() => {
  ...
  function nextStep(todo, run) {
    ...
    if (run.phase === 'migration_required') return ['merge','Merge & restart',true];
  ...
  document.addEventListener('click', async event => {
    ...
    if (action === 'merge' && !confirm(`${run.repositories ? 'Integrate and publish all changed repositories in this UM run. ' : ''}Merge ${run.branch} at ${run.commit}, push it and restart the configured artifact?${testStatus(run) ? `\n\n${testStatus(run)}` : ''}`)) return;
