...
(() => {
  ...
  function render() {
    ...
    document.querySelectorAll('[data-workflow-next]').forEach(slot => {
      ...
      const html = `<button type="button" class="button small next-step" title="${['implement','retry'].includes(action) ? 'Publish task branches, including inherited local commits' : ''}" data-workflow-action="${action}" data-todo="${todo.id}" ${disabled ? 'disabled' : ''}>${label}${allowed ? ' ↗' : ''}</button>`;
